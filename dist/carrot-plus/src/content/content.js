...
async function sendRuntimeMessageWithRetries(message, retries = MESSAGE_RETRIES) {
  let lastError;
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      lastError = er;
      if (!isMessageChannelClosedError(er) || attempt === retries) {
        throw er;
      }
      console.warn(
        '[Carrot] Runtime message failed, retrying %s/%s: %o',
        attempt,
        retries,
        er);
...
function getHandleFromStandingsRow(tableRow) {
  const contestantCell = tableRow.querySelector('td.contestant-cell');
  if (!contestantCell) {
    return null;
  }
  ...
  return contestantCell.textContent.trim();
}
...
function updateStandings(resp) {
  ...
  for (const [idx, tableRow] of rows.entries()) {
    ...
    if (idx === 0) {
      ...
      const handle = getHandleFromStandingsRow(tableRow);
      populateCells(
        handle ? resp.rowMap[handle] : undefined,
        resp.type,
        rankUpTint,
        perfCell,
        deltaCell,
...
async function predict(contestId) {
  const response = await sendRuntimeMessageWithRetries({ type: 'PREDICT', contestId });
...
const API_PATH = '/api/';
const API_FETCH_RETRIES = 20;
const API_FETCH_TIMEOUT_MS = 20 * 1000;
...
async function fetchWithRetries(url) {
  ...
  for (let attempt = 1; attempt <= API_FETCH_RETRIES; attempt++) {
    ...
    try {
      ...
      lastError = er;
      console.warn(
        '[Carrot] CF API fetch failed, attempt %s/%s: %o',
        attempt,
        API_FETCH_RETRIES,
        er);
  ...
  if (contestId && document.querySelector('table.standings')) {
    predict(contestId)
      ...
      .catch(er => {
        ...
        sendRuntimeMessageWithRetries({ type: 'SET_ERROR_BADGE' }, 2).catch(console.warn);
      });
  }

  // On any Codeforces page.
  const ping = () => {
    sendRuntimeMessageWithRetries({ type: 'PING' }, 2).catch(console.warn);
  };
  ping();
